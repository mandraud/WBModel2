##########################################################################################################
####################                        WB Model + Hunting variable 40% & 70%            ##################
...
WBModel <- function( MaxIterations    = 2,
                     MaxDays          = 365*5,
                     ...
                     runID            = paste0("WB_MovH", ProbMovHunt*100, "_Groups_", InitialOccCells, "ASF")
){
  ...
  for(Iter in 1:MaxIterations){
    ...
    # PopMatWB <- matrix(0, ncol = 16, nrow = 1)
   ...
   if(gTime <= TimeSeedInf) Criteria <- TRUE else Criteria <- any(PopMatWB[ ,9] %in% c(1:4))
      ...
      if(gTime %in% c(1, (366 * 1:(MaxDays/365)))){
        ...
        BreedCapCells   <- c(WBMat[PopMatWB[ ,7], 15])
        ...
        AniAllowBreed   <- AnimalsWInG <= BreedCapCells & PopMatWB[ ,3] == 1 & PopMatWB[ ,4] > 1 & PopMatWB[ ,9] < 2
...
# REPRODUCTION ------------------------------------------------------------
      ...
      ## On daily basis, from Jan to end June, check if there are animals to deliver and make them deliver
        ...
        DelIndex        <- which(PopMatWB[ ,6] == gTime & PopMatWB[ ,9] != 3)
...
# FEMALE GROUP SPLITTING --------------------------------------------------
      ...
      if(gTime %in% ((28*7 + ((Year - 1)*365)) + 0:6)){
        ...
        BreedingAgeFemales <- PopMatWB[ ,4] >= 2 & PopMatWB[ ,3] == 1 &  # Age category 2: Subadults   # Sex:females
                              PopMatWB[,10] == 0 &  PopMatWB[,9] < 2     # Didn't move yet  # Not infected or in latency 
        cellBreeders     <- do.call(rbind,sapply(unique(PopMatWB[ ,2]), 
                                                  function(x){ 
                                                  if (any(index<-PopMatWB[BreedingAgeFemales,2] == x)){
                                                      cbind(group=x,
                                                            HomeRange=PopMatWB[BreedingAgeFemales & PopMatWB[,2] == x,7],
                                                            AnimalsWInG=1:sum(PopMatWB[BreedingAgeFemales,2] == x))
                                                  }else{cbind(group=x,HomeRange=unique(PopMatWB[PopMatWB[,2] == x,7]),AnimalsWInG=0)}
                                                  }) 
                                                     
                                    )
        BreedCapCells   <- WBMat[match(cellBreeders[,2],WBMat$ID),15]
        cellBreeding=data.frame(cellBreeders,BreedCapCells)  
        ...
        # subset()
        # subset(PopMatWB,)
        GroupSplitNum   <- cbind(sort(unique(cellBreeding$group)), tapply(cellBreeding$AnimalsWInG > cellBreeding$BreedCapCells,
                                                                          as.factor(cellBreeding$group), sum))
                                                                            # PopMatWB[ ,4] == 2 &  # Age category 2: Subadults
                                                                            # PopMatWB[ ,3] == 1 &  # Sex:females
                                                                            # PopMatWB[,10] == 0 &  # Didn't move yet
                                                                            # PopMatWB[,9] < 2),    # Not infected or in latency   
        ...
        SlitPopOrigin=PopMatWB[match(GroupSplitNum[ ,1], PopMatWB[ ,2]), 7]
        
        ### Keep moving groups with suitable habitat neighbourhood and no females within
        GroupsSplitShD  <- sapply(SlitPopOrigin, function(x) {
          tmp1 <-which(Distance[x,] <= DistThreshold)
          any(WBMat[tmp1, 5] == 1 & !(tmp1 %in% PopMatWB[!PopMatWB[ ,2] %in% PixelMalesOnly[ ,1], 7]))})
        GroupSplitNumSD <- GroupSplitNum[which(GroupsSplitShD),]
        ...
        if(dim(GroupSplitNumSD)[1] > 0){
          ...
          if(dim(GroupSplitNumSD)[1] > 0){
            ...
            PixelsMoved <-  vector(mode = "list", length =length(TargetPixel))
            
            
            CurrentPos <- OriginPixel
            for(i in seq_along(TargetPixel)){
              Trail <- 0
            ...
            newGroupIDs <- max(PopMatWB[ ,2])+as.numeric(factor(GroupSplitNumSD[ ,3]))
              ...
              for(b in 1:dim(GroupSplitNumSD)[1]){
                ...
                IndexMalComb1  <- GroupSplitNumSD[b, 3] %in% PopMatWB[ ,7]
                if(IndexMalComb1){
                  IndexMalComb2  <- unique(PopMatWB[PopMatWB[ ,7] == GroupSplitNumSD[b, 3], 2]) %in% GroupsToSplit[ ,1]
                  if(!IndexMalComb2){
                    ## select the subpopulation in group b 
                    AnimalsCanSplitF <- which((PopMatWB[ ,2] %in% GroupSplitNumSD[b, 1]))
                    
                    ##Select the splitting animals: These are females but keeping a breeding capacity in the cell:
                    #   the PopMatWB matrix is ordered per groups such as females appear first in decreasing age order 
                    #   therefore we can keep the first "breedingCapacity" sows. Then the next ones are moving 
                    #   up to the variable GroupSplitNumSD[b, 2], meaning all extra mature and subadults females.
                    groupBreedCap<-unique(cellBreeding$BreedCapCells[cellBreeding$group==GroupSplitNumSD[b, 1]])
                    AnimalsCanSplitF <- AnimalsCanSplitF[groupBreedCap+(1:GroupSplitNumSD[b, 2])]  
                    
                    MalesInPixel     <- which(PopMatWB[ ,3] == 0 & 
                                        PopMatWB[ ,9] != 3 & 
                                        PopMatWB[ ,7] == GroupSplitNumSD[b, 3] & 
                                       !PopMatWB[ ,2] %in% GroupsToSplit[ ,1])
                    
                    
                    newGroupIDsAn    <- rep(newGroupIDs[b], length(AnimalsCanSplitF) + length(MalesInPixel)) 
                    NewHomePixel     <- rep(GroupSplitNumSD[b, 3], length(AnimalsCanSplitF) + length(MalesInPixel))
                    PopMatWB[c(AnimalsCanSplitF,MalesInPixel), 2]  <- newGroupIDsAn
                    PopMatWB[c(AnimalsCanSplitF,MalesInPixel), 7]  <- NewHomePixel
                    PopMatWB[c(AnimalsCanSplitF,MalesInPixel), 8]  <- NewHomePixel
                  }
                  if(IndexMalComb2){
                    ## select the subpopulation in group b 
                    AnimalsCanSplitF <- which(PopMatWB[ ,2] %in% GroupSplitNumSD[b, 1])
                    ...
                    groupBreedCap<-unique(cellBreeding$BreedCapCells[cellBreeding$group==GroupSplitNumSD[b, 1]])
                    AnimalsCanSplitF <- AnimalsCanSplitF[groupBreedCap+(1:GroupSplitNumSD[b, 2])]  
                    newGroupIDsAn    <- rep(newGroupIDs[b], length(AnimalsCanSplitF)) 
                    NewHomePixel     <- rep(GroupSplitNumSD[b, 3], length(AnimalsCanSplitF))
                ...
                if(!IndexMalComb1){
                  ...
                  ##assign newgroup ID to splitting animals
                  PopMatWB[AnimalsCanSplitF, 2]  <-rep(newGroupIDs[b], length(AnimalsCanSplitF)) 
                  
                  ##Assign new Homepixel to splitting animals
                  PopMatWB[AnimalsCanSplitF, 8]  <- PopMatWB[AnimalsCanSplitF, 7] <- rep(GroupSplitNumSD[b, 3], length(AnimalsCanSplitF))
